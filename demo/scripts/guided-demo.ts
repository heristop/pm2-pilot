...
const PM2X_PATH = path.resolve(DEMO_DIR, '../dist/bin/pm2x.js');
...
const GUIDANCE_STEPS: GuidanceStep[] = [
  ...
  },
  {
    ...
    hints: [
      "PM2-X can understand natural language queries",
  ...
  },
  {
    ...
    hints: [
      "PM2-X provides prioritized recommendations",
...
class GuidedDemo {
  ...
  async startDemoProcesses(): Promise<void> {
    console.log(chalk.blue('🚀 Starting PM2-X Guided Demo...'));
  ...
  showWelcome(): void {
    ...
    console.log(chalk.cyan('║') + chalk.white.bold('                         🎯 PM2-X Guided Demo                                  ') + chalk.cyan('║'));
    ...
    console.log(chalk.gray('   • Natural language interaction with PM2-X'));
  ...
  async launchPM2X(): Promise<void> {
    console.log(chalk.blue('🚀 Launching PM2-X with guidance...\n'));
    ...
    this.pm2xProcess.on('close', (code) => {
      console.log(chalk.blue('\n🎉 Thanks for completing the PM2-X Guided Demo!'));
      console.log(chalk.gray('You\'ve learned how to use PM2-X for process management and error analysis.'));
      process.exit(code || 0);
    });
  ...
  nextStep(): void {
    if (this.currentStep < GUIDANCE_STEPS.length - 1) {
      ...
      console.log(chalk.gray('Continue exploring PM2-X or type "/exit" to finish the demo.'));
    }
  }
