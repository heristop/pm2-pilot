Feature: PM2-X Demo - Orchestrated Showcase
  As a potential user of PM2-X
  ...
  Background: Demo Environment Setup
    Given I have a clean PM2-X demo environment
  ...
  @demo @main-flow
  Scenario: Complete PM2-X Demo Flow (50 seconds)
    # Scene 1: Status Overview (0-10s)
    When I run the PM2-X command "/status"
    ...
    And I run the PM2-X command "/logs smart api-server"
    ...
    When I pause for 3 seconds
    And I run the PM2-X command "/doctor logs"
    ...
    When I pause for 2 seconds
    And I run the PM2-X command "/exit"
  ...
  Scenario: Demo Performance Validation
    ...
    When I run any PM2-X command during the demo
